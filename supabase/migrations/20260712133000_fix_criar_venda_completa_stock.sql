...
AS $function$
declare
  v_user_id     uuid := get_owner_user_id(auth.uid());
  v_operador_id uuid := coalesce(nullif(p_payload->>'operador_id','')::uuid, auth.uid());
  v_venda_id bigint;
...
begin
  ...
  loop
    insert into public.vendas_itens (
      ...
      nome_produto_na_venda, preco_unitario_na_venda
    )
    values (
      v_user_id,
      v_venda_id,
  ...
  for v_linha in
    select c.id as id_categoria,
      ...
      from jsonb_to_recordset(coalesce(p_payload->'estoque','[]'::jsonb))
        as t(id_produto integer, quantidade integer)
  ...
  -- Audit log: only when a sub-user (different auth.uid from owner) made the sale
  if auth.uid() is distinct from v_user_id then
    insert into public.access_audit_logs (
      owner_user_id, operator_user_id, action, entity_type, entity_id, details
    ) values (
      v_user_id,
      auth.uid(),
      'venda.criada',
      ...
      jsonb_build_object(
        ...
        'forma_pagamento', coalesce(nullif(p_payload->>'forma_pagamento',''), 'dinheiro')
      )
    );
  end if;
...
drop function if exists public._migration_inspect_function(text);
