...
test('monta meio a meio e preserva escolhas após recarregar',async({page})=>{
 ...
 await dialog.getByRole('button',{name:/Grande.*até 2 sabores/}).click();
 await dialog.getByRole('button',{name:'2 sabores · partes iguais'}).click();
 await dialog.getByRole('button',{name:/Calabresa/}).click();
 await expect(dialog.getByRole('button',{name:'Escolha mais 1 sabor',exact:true})).toBeEnabled();
 await dialog.getByRole('button',{name:/Portuguesa/}).click();
...
test('edita pizza no carrinho e envia nova composição no autosave',async({page})=>{
 ...
 const dialog=page.getByRole('dialog');
 await expect(dialog.getByRole('button',{name:/Calabresa/})).toHaveAttribute('aria-pressed','true');
 await expect(dialog.getByRole('button',{name:/Portuguesa/})).toHaveAttribute('aria-pressed','true');
 await dialog.getByRole('button',{name:'Um sabor inteiro'}).click();
 await dialog.getByRole('button',{name:/Calabresa/}).click();
