...
export async function loadCatalogFromDb(empresaId: string): Promise<void> {
  ...
  const [categoriasRes, subcategoriasRes, produtosRes, publicationsRes, modifierGroupsRes, modifierOptionsRes, modifierOptionProductsRes] = await Promise.all([
    ...
    supabase.from('zelomenu_modifier_groups').select('id, id_produto, nome, tipo, modo_preco, min_selecoes, max_selecoes, ativo, ordem').eq('id_usuario', userId).order('ordem').limit(4000),
