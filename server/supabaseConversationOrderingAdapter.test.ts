...
describe('migration de snapshots parciais', () => {
  it('instala um trigger terminal antes da constraint de prontidao', () => {
    const sql = readFileSync(
      'supabase/history/conversation-ordering/20260902110000_conversation_ordering_partial_snapshots.sql',
      'utf8',
    );
...
describe('fixture de integridade da confirmação conversacional', () => {
  ...
  it('usa hashes SHA-256 hexadecimais literais e distintos para emissão', () => {
    ...
    // ZM2 adds four otherwise-ready invalid-token fixtures (wrong, expired,
    // consumed and wrong-revision) on top of the existing 16 issuances.
    expect(issuedHashes).toHaveLength(20);
...
describe('migration de fencing por epoch', () => {
  it('trava o controle compartilhado real e cerca todas as escritas com RPCs server-only', () => {
    const sql = readFileSync(
      'supabase/history/conversation-ordering/20260902130000_fence_conversation_ordering_with_ai_epoch.sql',
      'utf8',
    );
